...
void test(const net::transfer_exactly &trans, size_t sz ) // pass as const ref tests that op() is const
{
	...
	assert ( trans(noErr, sz - 100) <= 100 );
	assert ( trans(noErr, sz -   1) <=   1 );
...
// 	static_assert(std::is_same<net::transfer_all()::second_argument_type, size_t>::value, "" );
	...
	test(net::transfer_exactly(1000), 1000);
}
