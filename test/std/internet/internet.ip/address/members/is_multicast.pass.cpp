...
int main()
{
    static_assert ( noexcept(ip::address().is_multicast()), "" );
    ...
    assert(!addr0.is_multicast());
    assert( addr1.is_multicast() == v4addr1.is_multicast());
    assert( addr2.is_multicast() == v4addr2.is_multicast());
    }
    {
    ...
    assert(!addr0.is_multicast());
    assert( addr1.is_multicast() == v6addr1.is_multicast());
    assert( addr2.is_multicast() == v6addr2.is_multicast());
    }
